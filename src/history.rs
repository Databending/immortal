...
impl History {
    ...
    pub async fn get_workflows(
        ...
        let mut workflows: Vec<_> = workflows
            .into_iter()
            .filter(|f| f.is_some())
            .map(|x| unsafe { simd_json::from_str(&mut x.unwrap()).unwrap() })
            .collect();
        // .filter(|f| match f {
...
impl ToRedisArgs for WorkflowHistoryVersion {
        ...
        W: ?Sized + RedisWrite,
    {
        ...
        let serialized = simd_json::to_string(self).expect("Failed to serialize to JSON");
    ...
    fn from_redis_value(value: &redis::Value) -> Result<Self, RedisError> {
        match value {
            ...
            redis::Value::BulkString(x) => {
                let mut x = x.clone();
                match simd_json::from_slice(&mut x) {
                    Ok(data) => {
                        return Ok(data);
                    }
                    Err(_e) => {
